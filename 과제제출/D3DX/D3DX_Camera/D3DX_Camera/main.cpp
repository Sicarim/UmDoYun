...
LPDIRECT3DTEXTURE9		g_pTexDiffuse = NULL; // Texture 색깔맵(확산광)
...
VOID Animate()
{
	...
	// Z축 회전행렬
	D3DXMatrixRotationZ(&g_matAni, CameraAngle);
...
VOID Render()
{
	...
	if (SUCCEEDED(g_pd3dDevice->BeginScene()))
	{
		...
		pFont->DrawText(NULL, "상, 하: Q, E ", -1, &rc, DT_NOCLIP, D3DXCOLOR(0.0f, 1.0f, 1.0f, 1.0f));
		SetRect(&rc, 10, 30, 0, 0);
		pFont->DrawText(NULL, "앞, 뒤: W, S", -1, &rc, DT_NOCLIP, D3DXCOLOR(0.0f, 1.0f, 1.0f, 1.0f));
		SetRect(&rc, 10, 50, 0, 0);
		pFont->DrawText(NULL, "좌, 우: A, D", -1, &rc, DT_NOCLIP, D3DXCOLOR(0.0f, 1.0f, 1.0f, 1.0f));
		SetRect(&rc, 10, 70, 0, 0);
		pFont->DrawText(NULL, "Z(반시계)회전: Z", -1, &rc, DT_NOCLIP, D3DXCOLOR(0.0f, 1.0f, 1.0f, 1.0f));
